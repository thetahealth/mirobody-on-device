# The Mirobody brand mark, rendered with PIL. Shared by every platform's icon
# generator; import it, do not copy it.
#
# The mark is the two-lobe glyph the web client, android, and HarmonyOS all show --
# paths copied verbatim from htdoc/src/assets/mirobody.svg (the single source; see
# docs/colors-and-fonts.md S5). Keeping the flattener here means a fix to the curve
# quality or the proportions lands on all clients at once, which is the whole reason
# it is not inlined per platform.
#
# Colors are FIXED, not theme-aware: a launcher icon and an .exe icon have no theme.
# The upper lobe takes brand_logo_mark's light value, the lower one the Theta Health
# blue that never varies by mode. (android's launcher foreground hardcodes #000000
# for the upper lobe rather than #0F1115 -- invisible in practice, but the sheet's
# value is the one worth copying.)
#
# SVG paths are flattened here rather than handed to an external rasterizer so this
# keeps needing nothing but PIL. Only the commands these two paths actually use are
# supported -- absolute M/C/H/V/Z; anything else raises rather than misdraw.
#
# This module only RETURNS images. Writing them where a platform wants them is the
# calling script's job (harmony/icon/gen_icon.py, qt/icon/gen_icon.py).
