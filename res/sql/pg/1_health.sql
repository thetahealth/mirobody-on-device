-- FHIR R5 resources, one row per (user, type, id). The full resource JSON is in
-- `content`; the columns beside it are the server-owned lifecycle fields the REST
-- layer needs without parsing the body (see src/fhir/):
--   version_id   monotonic version, bumped each write (FHIR meta.versionId)
--   updated_at   last write time, unix ms (app-stamped); search orders by it and
--                it backs the Last-Modified header (FHIR meta.lastUpdated, the
--                ISO instant, lives inside content)
--   deleted_at   soft delete: NULL = live, unix ms = when deleted (read -> 410 Gone)
