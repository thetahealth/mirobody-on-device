...
// Build an EVP_PKEY (RSA public key) from a JWK's base64url-encoded modulus and
// exponent. Returns nullptr on any parse / OpenSSL failure; caller owns the
// result and must EVP_PKEY_free.
...
// Extract a string field from a rapidjson object, returning nullptr if absent
// or not a string. Used to keep the JWKS / claims parsing readable.
...
// Parse `max-age=<N>` (in seconds) out of a Cache-Control header value and clamp
// into [min_secs, max_secs]. Returns `fallback` if the directive is absent or
// unparseable. Clamping defends against a misconfigured server pushing us into a
// never-refresh or hammer-on-every-call state.
...
struct AppleTokenValidator::Impl {
    using clock = std::chrono::steady_clock;

    // Fallback TTL when the response carries no usable Cache-Control. Apple does
    // send one in practice, but belt-and-suspenders: 1 hour is conservative if
    // it goes missing.
    static constexpr std::chrono::seconds kDefaultTtl{3600};
    // Clamp range so a misconfigured server can't push us into hammering (min)
    // or never refreshing (max). Cap at 24h.
    ...
    // Fetch the JWKS from Apple and rebuild the cache. Caller holds mu. Returns
    // true on success.
    bool refresh_locked() {
        client::HttpClient http;
        auto resp = http.get(kAppleJwkUrl, /*timeout_ms=*/10000);
        ...
        // Honor the response's Cache-Control max-age, falling back to the default
        // TTL when it's absent or unparseable. Clamped into [kMinTtl, kMaxTtl].
    ...
    // Look up the public key for `kid`. Refreshes the cache lazily when it's
    // expired, and once more if the kid still isn't known (Apple sometimes ships
    // a new signing key before the cache TTL elapses). Returns nullptr if no
...
AppleTokenValidator::AppleTokenValidator(std::string client_id)
    : impl_(new Impl(std::move(client_id))) {}

//------------------------------------------------------------------------------

AppleTokenValidator::~AppleTokenValidator() = default;
AppleTokenValidator::AppleTokenValidator(AppleTokenValidator&&) noexcept = default;
AppleTokenValidator& AppleTokenValidator::operator=(AppleTokenValidator&&) noexcept = default;

//------------------------------------------------------------------------------

AppleTokenValidator::VerifyResult
AppleTokenValidator::verify_token(const std::string& id_token) {
    VerifyResult r;

    if (id_token.empty()) { r.error = "Empty Apple token"; return r; }

    // Three segments separated by exactly two dots.
    const auto first_dot = id_token.find('.');
    if (first_dot == std::string::npos)  { r.error = "Malformed Apple token"; return r; }
    const auto second_dot = id_token.find('.', first_dot + 1);
    if (second_dot == std::string::npos) { r.error = "Malformed Apple token"; return r; }
    if (id_token.find('.', second_dot + 1) != std::string::npos) {
        r.error = "Malformed Apple token"; return r;
    }
    ...
    if (!header_bytes) { r.error = "Invalid Apple token header encoding"; return r; }

    rapidjson::Document header_doc;
    if (header_doc.Parse(header_bytes->c_str()).HasParseError() || !header_doc.IsObject()) {
        r.error = "Invalid Apple token header JSON";
        return r;
    }
    const char* alg = json_str(header_doc, "alg");
    if (!alg || std::string(alg) != "RS256") {
        r.error = "Apple token alg is not RS256";
        return r;
    }
    const char* kid = json_str(header_doc, "kid");
    if (!kid) {
        r.error = "Apple token missing kid";
        return r;
    }

    EVP_PKEY* pkey = impl_->get_key(kid);
    if (!pkey) {
        r.error = std::string("Apple JWKS lookup failed for kid: ") + kid;
        return r;
    }

    auto sig_bytes = b64url_decode(sig_b64);
    if (!sig_bytes) { r.error = "Invalid Apple token signature encoding"; return r; }
    ...
    if (!rsa_verify_sha256(pkey, signing_input, *sig_bytes)) {
        r.error = "Apple token verification failed: bad signature";
        return r;
    }

    auto payload_bytes = b64url_decode(payload_b64);
    if (!payload_bytes) { r.error = "Invalid Apple token payload encoding"; return r; }

    rapidjson::Document payload_doc;
    if (payload_doc.Parse(payload_bytes->c_str()).HasParseError() || !payload_doc.IsObject()) {
        r.error = "Invalid Apple token payload JSON";
        return r;
    }
    ...
    if (!exp)                  { r.error = "Apple token missing exp claim"; return r; }
    if (now_seconds() >= exp)  { r.error = "Apple token expired.";          return r; }

    // iss must be Apple.
    const char* iss = json_str(payload_doc, "iss");
    if (!iss) { r.error = "Apple token missing iss claim"; return r; }
    if (std::string(iss) != kAppleIssuer) {
        r.error = std::string("Apple token iss not from Apple: ") + iss;
        return r;
    }

    // aud must equal our client_id (the Services ID). Apple may send `aud` as a
    // single string (the common case for the web flow); that's all we accept.
    const char* aud = json_str(payload_doc, "aud");
    if (!aud) { r.error = "Apple token missing aud claim"; return r; }
    if (impl_->client_id != aud) {
        r.error = "Apple token invalid audience, expected: " + impl_->client_id;
        return r;
    }

    // email is optional: Apple only sends it on the *first* authorization (and
    // only if the client requested the `email` scope), so repeat sign-ins carry
    // none. We also trust it only when verified; otherwise the caller identifies
    // the account by the provider subject (`sub`). The full payload is returned
    // for the caller to read (email / email_verified / sub).
