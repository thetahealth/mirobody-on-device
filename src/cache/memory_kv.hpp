...
public:
    ...
    // call lazily initializes the singleton with thread-safe function-local static
    ...
    // (resulting value 1). Returns std::nullopt when the existing
    ...
    // otherwise std::nullopt. Lazily evicts on expiration. Mirrors
...
private:
    ...
    std::optional<std::int64_t> apply_delta_locked(const std::string& key, std::int64_t delta);
    std::optional<std::string>  pop_locked(const std::string& key, bool front);
