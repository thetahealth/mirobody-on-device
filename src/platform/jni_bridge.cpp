// Desktop/server JNI bridge — the in-process path for the JVM (HotSpot).
//
// Mirrors the Android bridge (android_jni.cpp) for the same Java class,
// ai.thetahealth.mirobody.NativeBridge, but is compiled for desktop JVMs and
// applies the database schema before serving (like main.cpp / c_api.cpp), so a
// host that just System.loadLibrary()s this and calls nativeStart gets a fully
// migrated database.
//
// Why this exists alongside the FFM binding in bindings/java: on Windows HotSpot
// the Panama FFM downcall into the embedded libwebsockets server crashes during
// lws_create_context (a JVM-signal/SEH interaction; the same library runs fine
// under Go and Python). JNI native calls enter the VM by a different route, so
// this bridge is the supported in-process option.
//
// The android_jni.cpp copy is guarded to __ANDROID__ and this one to
// !__ANDROID__, so the two definitions of the Java_… symbols never collide.
...
#else
#  error "jni_bridge: no migration connection for the selected database backend"
#endif
...
//------------------------------------------------------------------------------
// JNI exports — class ai.thetahealth.mirobody.NativeBridge
//------------------------------------------------------------------------------
...
Java_ai_thetahealth_mirobody_NativeBridge_nativeStart(
    ...
    if (!data_dir.empty())   cfg.sqlite.path = data_dir + "/mirobody.db";
