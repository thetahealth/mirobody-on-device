...
// silently downgrades the standard (e.g. an Android module forgetting
// to set `cppFlags`) before the error surfaces as a missing
// mirobody::optional (or another compat shim) deeper in.
//
...
// of this header from a project that forgot the flag still works.
    ...
    static_assert(_MSVC_LANG  >= 201103L, "mirobody requires C++11 or newer");
#else
    static_assert(__cplusplus >= 201103L, "mirobody requires C++11 or newer");
#endif
