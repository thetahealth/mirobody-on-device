// Standalone verifier for Firebase Auth ID tokens.
//
// Hands a JWT to jwt::FirebaseTokenValidator end-to-end (cert-bundle
// fetch + RS256 verify + iss/aud/exp/email checks) so the validator can
// be exercised without spinning up the server.
//
//   firebase_jwt "eyJhbGciOi..."
//   firebase_jwt --project-id my-firebase-project "eyJ..."
//   pbpaste | firebase_jwt
//
// Configuration precedence (highest to lowest):
//   1. command-line --flag
//   2. environment variable matching the YAML key (FIREBASE_PROJECT_ID)
...
void print_usage(const char* prog) {
    std::fprintf(stderr,
        ...
        "\n"
        "Verify a Firebase Auth ID token (RS256 JWT) against Google's\n"
        "securetoken cert bundle.\n"
        "\n"
        "Options:\n"
        "  --config <path>      Local YAML to load (overrides MIROBODY_CONFIG / ./config.yml).\n"
        "  --project-id <id>    Override FIREBASE_PROJECT_ID; required for the `aud` check.\n"
...
int parse_args(int argc, char** argv, Args& out) {
    ...
    out.project_id = store.get_str("FIREBASE_PROJECT_ID", out.project_id);
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--project-id")        { out.project_id = need(i, "--project-id"); }
    ...
    if (out.project_id.empty()) {
        std::fprintf(stderr,
            "FIREBASE_PROJECT_ID not set - pass --project-id, export FIREBASE_PROJECT_ID, "
...
constexpr const char* kCli = "firebase_jwt";
...
int main(int argc, char** argv) {
    ...
    mirobody::platform::log_info("%s: project_id=%s, token=%zu bytes",
        kCli, args.project_id.c_str(), args.id_token.size());

    mirobody::jwt::FirebaseTokenValidator validator{args.project_id};
