// Standalone debugger for GeminiLiveClient -- the Gemini Live API
// (BidiGenerateContent) over WebSocket, text-in / text-out. Both AI Studio and
// Vertex paths.
//
//   # AI Studio (default mode)
//   gemini_live --model gemini-2.0-flash-live-001 "summarize this"
//
//   # Vertex AI
//   gemini_live --mode vertex --gcp-project my-proj --gcp-location us-central1 "..."
//
// Config precedence: --flag > env > YAML (local / remote) > builtin default.
//
// YAML keys honored:
//   GEMINI_LIVE_MODE                ("ai-studio" or "vertex")
...
//   GEMINI_LIVE_AI_STUDIO_BASE_URL  override AiStudio host (wss://...)
//   GEMINI_LIVE_VERTEX_BASE_URL     override Vertex host (wss://...)
//   GEMINI_LIVE_API_VERSION         AiStudio service-name version (default v1beta)
//   GEMINI_LIVE_MODEL
//   GEMINI_LIVE_TEMPERATURE
//   GEMINI_LIVE_CONNECT_TIMEOUT_MS, GEMINI_LIVE_REQUEST_TIMEOUT_MS
//   GEMINI_LIVE_INPUT_PRICE, GEMINI_LIVE_OUTPUT_PRICE,
//   GEMINI_LIVE_MIN_CHUNK_SIZE
...
void print_usage(const char* prog) {
    std::fprintf(stderr,
        ...
        "\n"
        "Standalone Gemini Live API (BidiGenerateContent) debug client, text mode.\n"
        ...
        "  --mode <ai-studio|vertex>  Override GEMINI_LIVE_MODE. Default: ai-studio\n"
        ...
        "  --model <name>             Override GEMINI_LIVE_MODEL. Default: gemini-2.0-flash-live-001\n"
        "  --api-version <ver>        AiStudio service-name version. Default v1beta.\n"
...
bool parse_mode(const std::string& s, mirobody::llm::GeminiLiveMode& out) {
    if (s == "ai-studio" || s == "aistudio") { out = mirobody::llm::GeminiLiveMode::AiStudio; return true; }
    if (s == "vertex"    || s == "vertexai") { out = mirobody::llm::GeminiLiveMode::Vertex;   return true; }
    return false;
}
...
struct Args {
    mirobody::llm::GeminiLiveOptions opt;
...
int parse_args(int argc, char** argv, Args& out) {
    ...
    {
        std::string m = store.get_str("GEMINI_LIVE_MODE");
        if (!m.empty() && !parse_mode(m, out.opt.mode)) {
            std::fprintf(stderr, "GEMINI_LIVE_MODE must be ai-studio or vertex, got %s\n", m.c_str());
            return 2;
        }
    }
    ...
    out.opt.ai_studio_base_url = store.get_str("GEMINI_LIVE_AI_STUDIO_BASE_URL", out.opt.ai_studio_base_url);
    out.opt.vertex_base_url    = store.get_str("GEMINI_LIVE_VERTEX_BASE_URL",    out.opt.vertex_base_url);
    out.opt.api_version        = store.get_str("GEMINI_LIVE_API_VERSION",        out.opt.api_version);
    out.opt.model              = store.get_str("GEMINI_LIVE_MODEL",              out.opt.model);
    { std::string s = store.get_str("GEMINI_LIVE_TEMPERATURE"); if (!s.empty()) out.opt.temperature = std::atof(s.c_str()); }
    out.opt.connect_timeout_ms = static_cast<int>(
        store.get_int("GEMINI_LIVE_CONNECT_TIMEOUT_MS", out.opt.connect_timeout_ms));
    out.opt.request_timeout_ms = static_cast<int>(
        store.get_int("GEMINI_LIVE_REQUEST_TIMEOUT_MS", out.opt.request_timeout_ms));
    std::int64_t n = store.get_int("GEMINI_LIVE_MIN_CHUNK_SIZE", 0);
    if (n > 0) {
        out.opt.min_chunk_size = static_cast<size_t>(n);
    }
    { std::string s = store.get_str("GEMINI_LIVE_INPUT_PRICE"); if (!s.empty()) out.opt.input_price  = std::atof(s.c_str()); }
    { std::string s = store.get_str("GEMINI_LIVE_OUTPUT_PRICE"); if (!s.empty()) out.opt.output_price = std::atof(s.c_str()); }
...
constexpr const char* kCli = "gemini_live";

const char* mode_name(mirobody::llm::GeminiLiveMode m) {
    return m == mirobody::llm::GeminiLiveMode::Vertex ? "vertex" : "ai-studio";
}
...
void log_effective(const Args& args) {
    ...
    if (args.opt.mode == mirobody::llm::GeminiLiveMode::AiStudio) {
...
int main(int argc, char** argv) {
    ...
    mirobody::llm::GeminiLiveClient client{std::move(args.opt)};
