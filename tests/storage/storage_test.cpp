...
S3Config sample_s3() {
    S3Config c;
    // AWS's canonical Signature V4 example credentials, from the published
    // SigV4 test suite (AKIDEXAMPLE / wJalr...EXAMPLEKEY, region us-east-1):
    //   https://docs.aws.amazon.com/general/latest/gr/signature-v4-test-suite.html
    // Fixed, well-known, and NOT real keys — using them lets the signing test
    // assert against AWS's documented expected signature.
...
OssConfig sample_oss() {
    OssConfig c;
    // Alibaba Cloud's published OSS signature-doc example secret
    // (OtxrzxIsfpFjA7SwPzILwy8Bw21TLhquhboDYROV, paired in those docs with
    // AccessKeyId 44CF9590006BF252F707):
    //   https://www.alibabacloud.com/help/en/oss/developer-reference/include-a-signature-in-the-authorization-header
    // "LTAI" is the real Alibaba AccessKey-ID prefix and
    // oss-cn-hangzhou.aliyuncs.com is the Hangzhou region endpoint. Not real
    // keys — a fixed vector for the HMAC-SHA1 signing test.
    c.access_key_id     = "LTAIexample";
    c.secret_access_key = "OtxrzxIsfpFjA7SwPzILwy8Bw21TLhquhboDYROV";
    c.endpoint          = "oss-cn-hangzhou.aliyuncs.com";
    c.bucket            = "my-bucket";
    c.prefix            = "charts";
    return c;
}

AzureBlobConfig sample_azure() {
    AzureBlobConfig c;
    c.account   = "myaccount";
    c.key       = "QXp1cmVLZXkxMjM0NTY3ODkw";   // base64("AzureKey1234567890") -- not a real key
    c.container = "uploads";
    c.prefix    = "charts";
    return c;
}
...
TEST_CASE("S3Config::open rejects incomplete config", "[storage][s3]") {
    S3Config c;                       // empty
    REQUIRE_THROWS_AS(c.open(), StorageError);
    c.access_key = "k"; c.secret_key = "s"; c.bucket = "b";
    REQUIRE_THROWS_AS(c.open(), StorageError);   // region still missing
    c.region = "us-east-1";
...
TEST_CASE("AwsS3 builds virtual-hosted public and presigned URLs", "[storage][s3]") {
    AwsS3 s3(sample_s3());

    REQUIRE(s3.public_url("obj.png") ==
            "https://my-bucket.s3.us-east-1.amazonaws.com/charts/obj.png");
    // full_key is idempotent: a key already carrying the prefix isn't
    // prefixed again, so API-returned keys can be fed back in.
    REQUIRE(s3.full_key("obj.png") == "charts/obj.png");
    REQUIRE(s3.full_key("charts/obj.png") == "charts/obj.png");
    REQUIRE(s3.public_url("charts/obj.png") == s3.public_url("obj.png"));

    const std::string url = s3.presigned_url("obj.png", 900);
    REQUIRE(contains(url, "https://my-bucket.s3.us-east-1.amazonaws.com/charts/obj.png?"));
    REQUIRE(contains(url, "X-Amz-Algorithm=AWS4-HMAC-SHA256"));
    REQUIRE(contains(url, "X-Amz-Credential=AKIDEXAMPLE%2F"));   // slash-encoded scope
    REQUIRE(contains(url, "X-Amz-Expires=900"));
    REQUIRE(contains(url, "X-Amz-SignedHeaders=host"));
    REQUIRE(contains(url, "&X-Amz-Signature="));

    // The signature is a 64-char lowercase-hex SHA-256 HMAC.
    const auto pos = url.find("X-Amz-Signature=");
    const std::string sig = url.substr(pos + std::string("X-Amz-Signature=").size());
    REQUIRE(sig.size() == 64);
...
TEST_CASE("AwsS3 CDN public URL bypasses the bucket host", "[storage][s3]") {
    S3Config c = sample_s3();
    c.cdn = "cdn.example.com";
    AwsS3 s3(c);
    REQUIRE(s3.public_url("a/b.png") == "https://cdn.example.com/charts/a/b.png");
}

//------------------------------------------------------------------------------

TEST_CASE("AwsS3 path-style endpoint puts the bucket in the path", "[storage][s3]") {
    S3Config c = sample_s3();
    c.endpoint = "https://minio.local:9000";
    AwsS3 s3(c);
    REQUIRE(s3.public_url("obj.png") == "https://minio.local:9000/my-bucket/charts/obj.png");
}

//------------------------------------------------------------------------------

TEST_CASE("OssConfig::open rejects incomplete config", "[storage][oss]") {
    OssConfig c;
    REQUIRE_THROWS_AS(c.open(), StorageError);
    c.access_key_id = "k"; c.secret_access_key = "s"; c.bucket = "b";
    REQUIRE_THROWS_AS(c.open(), StorageError);   // endpoint still missing
    c.endpoint = "oss-cn-hangzhou.aliyuncs.com";
    REQUIRE_NOTHROW(c.open());
}
...
TEST_CASE("AliyunOss builds virtual-hosted public and presigned URLs", "[storage][oss]") {
    AliyunOss oss(sample_oss());

    REQUIRE(oss.public_url("obj.png") ==
            "https://my-bucket.oss-cn-hangzhou.aliyuncs.com/charts/obj.png");

    const std::string url = oss.presigned_url("obj.png", 600);
    REQUIRE(contains(url, "https://my-bucket.oss-cn-hangzhou.aliyuncs.com/charts/obj.png?"));
    REQUIRE(contains(url, "OSSAccessKeyId=LTAIexample"));
    REQUIRE(contains(url, "&Expires="));
    REQUIRE(contains(url, "&Signature="));
}

//------------------------------------------------------------------------------

TEST_CASE("AliyunOss custom domain public URL", "[storage][oss]") {
    OssConfig c = sample_oss();
    c.cdn = "static.example.com";
    AliyunOss oss(c);
    REQUIRE(oss.public_url("obj.png") == "https://static.example.com/charts/obj.png");
}

//------------------------------------------------------------------------------

TEST_CASE("azure_signature matches the RFC 4231 HMAC-SHA256 vector", "[storage][azure]") {
    // The Azure signature is base64(HMAC-SHA256(base64-decode(key), text)); RFC
    // 4231 test case 2 (key "Jefe", data "what do ya want for nothing?") pins the
    // whole base64-decode -> HMAC -> base64 chain to a known value.
    REQUIRE(mirobody::storage::base64_decode("SmVmZQ==") == "Jefe");
    REQUIRE(mirobody::storage::base64_encode("Jefe") == "SmVmZQ==");
    REQUIRE(mirobody::storage::azure_signature("SmVmZQ==", "what do ya want for nothing?")
            == "W9zBRr9gdU5qBCQmCJV1x1oAPwidJzmDnexYuWTsOEM=");
}

//------------------------------------------------------------------------------

TEST_CASE("AzureBlob builds blob URLs and a service SAS", "[storage][azure]") {
    AzureBlob az(sample_azure());

    REQUIRE(az.public_url("obj.png") ==
            "https://myaccount.blob.core.windows.net/uploads/charts/obj.png");
    // full_key applies the prefix idempotently: both forms resolve the same.
    REQUIRE(az.public_url("charts/obj.png") ==
            "https://myaccount.blob.core.windows.net/uploads/charts/obj.png");

    const std::string url = az.presigned_url("obj.png", 600);
    REQUIRE(contains(url, "https://myaccount.blob.core.windows.net/uploads/charts/obj.png?"));
    REQUIRE(contains(url, "sv=2021-08-06"));
    REQUIRE(contains(url, "&sr=b"));
    REQUIRE(contains(url, "&sp=r"));
    REQUIRE(contains(url, "&se="));
    REQUIRE(contains(url, "&sig="));
}

//------------------------------------------------------------------------------

TEST_CASE("AzureBlob honors endpoint suffix and CDN", "[storage][azure]") {
    AzureBlobConfig c = sample_azure();
    c.endpoint_suffix = "core.chinacloudapi.cn";   // a sovereign cloud
    REQUIRE(AzureBlob(c).public_url("obj.png") ==
            "https://myaccount.blob.core.chinacloudapi.cn/uploads/charts/obj.png");

    c.cdn = "cdn.example.com";
    REQUIRE(AzureBlob(c).public_url("obj.png") == "https://cdn.example.com/charts/obj.png");
}

//------------------------------------------------------------------------------

TEST_CASE("AzureBlobConfig::open rejects incomplete config", "[storage][azure]") {
    AzureBlobConfig c;
    REQUIRE_FALSE(c.configured());
    REQUIRE_THROWS_AS(AzureBlob(c), StorageError);
    c.account = "a"; c.key = "QQ=="; c.container = "c";
